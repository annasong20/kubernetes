...
func validatePodResources(spec *core.PodSpec, podClaimNames sets.Set[string], fldPath *field.Path, opts PodValidationOptions) field.ErrorList {
	...
	if spec.OS != nil && spec.OS.Name == core.Windows {
		...
		return field.ErrorList{
			field.Forbidden(resourcesPath, fmt.Sprintf("resources cannot be set when %q is %q", fldPath.Child("os").Child("name"), core.Windows)),
		}
	}
	...
	if spec.Resources.Claims != nil {
		allErrs = append(allErrs, field.Forbidden(resourcesPath.Child("claims"), "claims cannot be set for Resources at pod-level"))
	}

	// validatePodResourceRequirements checks if resource names and quantities are
	// valid, and requests are less than limits.
	allErrs = append(allErrs, validatePodResourceRequirements(spec.Resources, podClaimNames, resourcesPath, opts)...)
	allErrs = append(allErrs, validatePodResourceConsistency(spec, resourcesPath)...)
	return allErrs
}
